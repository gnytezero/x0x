...
- **Slow-consumer Close(1013) survives the writer's flush budget (#287,
  ...
  exited and dropped the sink — tearing the TCP connection before the client
  had drained enough to ever see the close frame. Clients on hosts whose
  kernel socket buffers outlast the budget observed `Connection reset
  without closing handshake` instead of the documented close (reproduced by
  the PR #667 CI head after the round-1 hermeticity work removed the
  confounding self-update failure). The writer now returns the sink when it
  exits, and connection cleanup holds it open for a 3 s grace window while
  retrying the flush: a client that resumes draining anywhere inside the
  grace receives Close(1013) after its kernel backlog. The writer's budget
  is configurable (`[ws] slow_close_flush_ms`, default 2000) so the
  regression test exercises the writer-exited path deterministically without
  a slow host. New tests: unit
...
- **Task-claim 500 `task not found` after a visible read (#643).** A replica
  ...
  v0.42.0 v1 snapshots still load, degrading safely to no evidence, and a
  list with no removal evidence keeps writing the v1 envelope so a v0.42.0
  rollback can still read it). Old peers' serves carry no evidence and prune nothing (safer than the old
...
- **Regression gate: `tests/gui_script_syntax.rs`.** The extracted inline
  script is compiled with a REAL JavaScript parser, not a hand-rolled scanner:
  telling a regex literal from division needs full parser context, so any
  heuristic either rejects valid code (`return /[)]/.test(s)`) or accepts
  invalid code (`i++ / (x;`). It uses `node:vm`'s `Script` — the browser
  classic-script grammar the GUI is actually served under — rather than
  `node --check`, whose CommonJS grammar diverges in both directions (accepting
  top-level `return;`, rejecting `let require = 1;`); both divergences are
  pinned by a test. Every GitHub-hosted runner image ships Node; if Node is
  absent the test fails loudly rather than skipping, so the gate cannot pass
  vacuously. Companion tests prove it rejects the exact v0.41.1 defect shape and
  accepts valid regexes, division and nested template literals. The existing GUI suites match endpoint strings and never checked
...
- **Release CI gate (#476):** the release workflow's `Require green CI on tagged commit` poll ceiling is raised from 20 to 45 minutes — the Coverage Gate now runs longer than the old ceiling, which forced a manual rerun on the v0.41.0 tag.
...
- Invite base rosters travel as a certificate-digest projection (roster cap 20 entries, derived from the exact link and DM-envelope budgets); `GroupMember` gains an authoritative `certificate_digest` used until certificate bytes hydrate from the announce cache.
- Peer-relay `RelayHeader` is bound to the inner payload (#437, #445); announce storm control (announce replay drop, per-author forward limits, broadcast-topic dedupe) (#421 #423 #425).
...
- Owner singleton and naming registry (ADR-0036, #430); Home auto-provisioning and primary agent (ADR-0038, #432 #435); agent harness boundary with rider tokens and actor context (ADR-0039, #439); signed delegation envelope, structured mentions, task claim/complete (ADR-0040, #441); cross-machine owner-state sync Tier 1 (ADR-0041, #440); loss-tolerant datagram voice lane (ADR-0042, #431); agent key-move protocol (ADR-0037/0043, #444, roaming ceremony gated off); relay metering phase (ADR-0035, #427); V3 self-verifying announces with cert-blob fetch (#380, #416 #419); `fan_out` on public send (#296, #353).
- CLI argument-level parity with the Home Suite API and dispatch-to-wire parity tests (#455); complete API reference (#454); SKILL.md Home Suite coverage (#453); `x0x group join --home --owner`, `x0x home` shows `owner_user_id`.
